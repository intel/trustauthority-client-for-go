...
var verifyCmd = &cobra.Command{
	...
	Short: "Verify Amber attestation token",
	Long:  ``,
...
func init() {
	...
	verifyCmd.Flags().StringP(constants.ConfigOption, "c", "", "Amber config in JSON format")
...
func verifyToken(cmd *cobra.Command) error {
	...
	if config.AmberUrl == "" {
		return errors.New("Amber URL is missing in config")
	}
	...
	parsedToken, err := amberClient.VerifyToken(string(token))
	if err != nil {
