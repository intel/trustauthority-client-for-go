/*
 *   Copyright (c) 2022 Intel Corporation
 *   All rights reserved.
 *   SPDX-License-Identifier: BSD-3-Clause
 */

package constants

const (
	RSAKeyBitLength     = 3072
	LinuxFilePathSize   = 4096
	CLIShortDescription = "Intel® Trust Authority CLI for TDX"
)

// Command Names
const (
	CreateKeyPairCmd    = "create-key-pair"
	DecryptCmd          = "decrypt"
	QuoteCmd            = "quote"
	TokenCmd            = "token"
	EvidenceCmd         = "evidence"
	AttestCmd           = "attest"
	RootCmd             = "trustauthority-cli"
	VersionCmd          = "version"
	VerifyCmd           = "verify"
	AkHandleOption      = "ak"
	PcrSelectionsOption = "pcrs"
)

// Options Names
const (
	PrivateKeyPathOption       = "key-path"
	PublicKeyPathOption        = "pub-path"
	PrivateKeyOption           = "key"
	PolicyIdsOption            = "policy-ids"
	InputOption                = "in"
	UserDataOption             = "user-data"
	NonceOption                = "nonce" // TODO: deprecate and use user-data?  --user-data 0xdecafbad --user-data <base64>
	ConfigOption               = "config"
	RequestIdOption            = "request-id"
	TokenAlgOption             = "token-signing-alg"
	PolicyMustMatchOption      = "policy-must-match"
	NoEventLogOption           = "no-eventlog"
	TokenOption                = "token"
	WithTpmOption              = "tpm"
	WithTdxOption              = "tdx"
	WithAzTdxOption            = "aztdx"
	ExcludeVerifierNonceOption = "no-verifier-nonce"
)
